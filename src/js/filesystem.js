...
function expectResponse({ tag, type }, data, send) {
  ...
  const pendingResponse = data.shift();
      ...
      .then((res) => {
        res.type === type
          ...
          : send({
              tag,
              $: "Err",
              message: `Expecting response of type ${type} but got ${res.type} instead.`,
            });
      })
      .catch((err) => {
...
function makeDir({ path }) {
  Filesystem.promises.mkdir(path, { recursive: true }).catch(() => {});
}

/* */
function readDir({ path }, data) {
  const pendingResponse = Filesystem.promises
    .readdir(path)
...
function readFile({ path, filename }, data) {
  ...
  const pendingResponse = Filesystem.promises
    ...
    .then((contents) =>
      Promise.resolve({ $: "GotFile", type: "File", contents })
    );
